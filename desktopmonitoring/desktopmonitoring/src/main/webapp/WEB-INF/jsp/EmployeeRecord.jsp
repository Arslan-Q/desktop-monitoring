...
function dismiss(){
	$.ajax({
		type : 'get',
		url : '${pageContext.request.contextPath}/dismiss',
		data : {
        	id:$("#inputid").val()
        	},
		success : function(data) {
			$('#btn_dismiss').text(data);
		}
	});
				...
				<div class="menu-sidebar__content js-scrollbar1">
					<nav class="navbar-sidebar">
						<ul class="list-unstyled navbar__list">
							...
							<li><a href="${pageContext.request.contextPath}/Record">
									<i class="fa fa-group"></i>Record
							</a></li>
		...
		<div class="page-container">
			...
			<div class="main-content">
				<div class="section__content section__content--p30">
					<div class="container-fluid">
						<div class="container">
							<h3 style="font-family: Arial Black, Times, serif;">${msg}</h3>
							<h3 style="font-family: Times New Roman, Times, serif;">Name
								:${firstname}&nbsp${lastname}</h3>
							<h5 style="font-family: Times New Roman, Times, serif;">email:${email}</h5>
							<form>
								<input type="hidden" value="${id}" id="inputid" name="inputid">
								<button type="button" class="btn btn-danger" id="btn_dismiss"
									style="margin-left: 70%" onclick="dismiss()">${btnAction}</button>
							</form>
							<table class="table table-bordered table-hover table-striped"
								...
								<c:forEach var="list" items="${employesslist}">
									<tr>
										<td class="m_title">${list.date}</td>
										<td><c:set var="total" value="${list.work+list.nonwork}" />
											<c:set var="work" value="${list.work/total}" /> <fmt:formatNumber
												var="workper" value="${work*100}" minFractionDigits="0"
												maxFractionDigits="0" /> <c:set var="nonwork"
												value="${list.nonwork/total}" /> <fmt:formatNumber
												var="nonworkper" value="${nonwork*100}"
											...
											<div class="progress">
												<div
													...
													aria-valuemax="100" style="width:${workper}%">
													${workper}% work</div>
											</div>
											<div class="progress">
												<div
													...
													aria-valuemax="100" style="width:${nonworkper}%">
													${nonworkper}% nonwork</div>
											</div></td>
										<td><form
												action="${pageContext.request.contextPath}/sendEmail"
												method="post">
												<input type="hidden" name="email_address"
													value="${list.email}"> <input
													class="btn btn-warning" type="submit" value="Warn">
											</form></td>
									</tr>
								</c:forEach>
